...
18-5-2019: Fix EEPROM Problem (v3)
...
unsigned int    Status=1;          //Vua bat nguon thi he thong se hoat dong
unsigned int    Mode=0;
unsigned int    count_temp1=99;
unsigned int    count_temp0=99;
unsigned long int count_save=0;
...
Timer_INIT();
INT0_INIT();
count_temp0 = eeprom_read_word(14);   //Load value from EEPROM
count_temp1 = eeprom_read_word(18);
if(count_temp0 == 0) count_temp0 = 99; //If read EEPROM=0 -> run at 99 dangerous ->down to 00
if(count_temp1 == 0) count_temp1 = 99; //If read EEPROM=0 -> run at 99 dangerous ->down to 00
SIGNAL = 0;
Status = 0;
Mode=0;
#asm("sei")
while (1)
      {  
        if(count_save>=15000000)      //auto save after 30min
        {
            eeprom_write_word(14,count_temp0);
            eeprom_write_word(18,count_temp1);
            count_save=0;
        }                
        else 
        {
    ...
    if(Mode == 1)
    {
        if(!SW_GIAM)
        {
            ...
            if(count_temp1 > 0)
            {
                count_temp1--;
                Delay_100us(5);
            } //Store count in EEPROM, EEPROM have only 100,000 write
            else
            {
                count_temp1 = 0;
            }
...
*/
    ...
    if((SW_EMER == 0)&&(SW_EMER1 == 0)||(SW_EMER == 1)&&(SW_EMER1 == 0))
    {
        Mode = 1;   //Mode 1
    }
    else
    ...
    if((SW_EMER == 1)&&(SW_EMER1 == 1)||(SW_EMER == 1)&&(SW_EMER1 == 0))
    {
         // LED = 1 ;
          SIGNAL = 0 ;
    }
    ...
    if(SW_EMER == 0)
    {
            if(Status == 0)                    //Switch press toggle the power
            {
                LED = 0;
            }
            else
...
//    {
//        LED = 0;
//        DDRB|= 0x02;   //System ON, on LED and enable SIGNAL
//    }
//    else
//    {
//        LED = 1;
//        DDRB&=~0x02;  //System OFF, Off LED and disable SIGNAL
//    }
}
/*--------------------------------------------------------*/
