...
17-12-2018: Change EEPROM library (Use built in EEPROM library and add delay before write to EEPROM)
//Test Section///////////////////////
*******************************************************/
...
SIGNAL=0;
#asm("sei")
count_temp0=eeprom_read_word(10);   //Load value from EEPROM
count_temp1=eeprom_read_word(12);
...
void CheckSW(void)  //TEST DONE
{
    if(Mode==1)
    {
        if(!SW_GIAM)
        {
            ...
            Delay_100us(10);
            eeprom_write_word(12,count_temp1);
        }
        if(!SW_TANG)
        {
            Delay_100us(2000);
    ...
    if(Mode==0)
    {
        if(!SW_GIAM)
        {
            ...
            Delay_100us(10);            
            eeprom_write_word(10,count_temp0);
        }
        if(!SW_TANG)
        {
            Delay_100us(2000);
