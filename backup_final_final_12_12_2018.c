...
//MAX 80us - 64895, shift 20 - Super strong
    ...
    TCNT1H=64895 >> 8;                 //FFB0-80us - (2^16-80)
    TCNT1L=64895 & 0xFF;
    //------------------------------------------------------//
    T_shift_delay++;
    if (Wait_Shift()==1)
...
Timer_INIT();
INT0_INIT();
count=eepromRead(0);   //Load value from EEPROM\ 
if(count==0) count=99; //If read EEPROM=0 -> run at 99 dangerous ->down to 0
SIGNAL=0;
#asm("sei")
while (1)
      {
...
{
    if(T_shift_delay>=20)           //sau 8*80us ve diem 0
    return 1;                       //Ve diem 0
    else                            //Chua ve diem 0
    return 0;
};
...
void CheckSW(void)  //TEST DONE
{
    ...
    {
        Delay_100us(2000);
        if(count>0) {count--;      Delay_100us(5);    eepromWrite(0,count);} //Store count in EEPROM, EEPROM have only 100,000 write
        else        count=0;
    }
    if(!SW_TANG)
    {
        Delay_100us(2000);
        if(count<99) { count++;     Delay_100us(5);    eepromWrite(0,count);} //Store count in EEPROM, EEPROM have only 100,000 write
        else         count=99;
    }
};
///////////////////////////////////////////////////////////
