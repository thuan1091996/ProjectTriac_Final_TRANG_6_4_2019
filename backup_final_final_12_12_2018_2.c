...
Timer_INIT();
INT0_INIT();
count_temp0=eepromRead(0);   //Load value from EEPROM
count_temp1=eepromRead(1);
...
void CheckSW(void)  //TEST DONE
{
    if(Mode==1)
    {
        ...
        {
            Delay_100us(2000);
            if(count_temp1>0) {count_temp1--;      Delay_100us(5);    eepromWrite(1,count_temp1);} //Store count in EEPROM, EEPROM have only 100,000 write
            else        count_temp1=0;
        }
        if(!SW_TANG)
        {
            Delay_100us(2000);
            if(count_temp1<99) { count_temp1++;     Delay_100us(5);    eepromWrite(1,count_temp1);} //Store count in EEPROM, EEPROM have only 100,000 write
            else        count_temp1=99;
        }
    }
    if(Mode==0)
    {
        ...
        {
            Delay_100us(2000);
            if(count_temp0>0) {count_temp0--;      Delay_100us(5);    eepromWrite(0,count_temp0);} //Store count in EEPROM, EEPROM have only 100,000 write
            else        count_temp0=0;
        }
        if(!SW_TANG)
        {
            Delay_100us(2000);
            if(count_temp0<99) {count_temp0++;     Delay_100us(5);    eepromWrite(0,count_temp0);} //Store count in EEPROM, EEPROM have only 100,000 write
            else        count_temp0=99;
        }
